        ...
        if($("#obstacles").children().eq(i).hasClass('bird')==false){
            ...
            if(obstacle_pos>-165){
                if($("#obstacles").children().eq(i).hasClass('boss') && boss_mouvement == true){
                    ...
                    if($("#obstacles").children().eq(i).hasClass('fire') && $("#obstacles").children().eq(i).length){ // FIRE
                        ...
                        if($("#obstacles").children().eq(i).hasClass("translated") == false &&  ( $("#obstacles").children().eq(i).hasClass("little1") || $("#obstacles").children().eq(i).hasClass("diam") || $("#obstacles").children().eq(i).hasClass("big") )  ){
                            get_obs_pos = $("#obstacles").children().eq(i).css("left").split("px").slice(0, -1) 
                            $("#obstacles").children().eq(i).css({"left":""+(get_obs_pos - 1000)+"px","transition":""+speed_time+"ms","transition-timing-function":"linear"}).addClass("translated")
                        }
                    }
                }
            }
                ...
                if($("#obstacles").children().eq(i).hasClass('boss')){
                    ...
                    if(boss_level < 6){
                        ...
                        bird_move_loop = setInterval(()=>{bird_move();bird_dead()},bird_move_key)
                        $("#obstacles").children().hide().remove()
                        ...
                        setTimeout(()=>{
                            ...
                            obstacle_move_loop = setInterval(()=>{ obstacle_move() ; bird_dead()},obstacle_move_key)
                        },500)
                    }
                }
                ...
                $("#obstacles").children().eq(i).css({"visibility":"hidden"})
                if($("#obstacles").children().eq(i).hasClass("boss")){
                    $("#obstacles").children().eq(i).removeClass().css({"position":"absolute"})
                }
            ...
            // loose condition 
                ...
                // GET DIAM
                    ...
                    $("#obstacles").children().eq(i).css({"visibility":"hidden"})
                    if($("#obstacles").children().eq(i).hasClass("boss") || $("#obstacles").children().eq(i).hasClass("diam")){
                        $("#obstacles").children().eq(i).removeClass().css({"position":"absolute"})
                    }
                    /*$("#obstacles").children().eq(i).remove()
                    obstacle.splice(i, 1)
...
let bird_move = function(){
        for(i=0;i<=obstacle.length;i++){
            ...
            if(obstacle_pos>-65){
                    // ADD BIRD
                    if($("#obstacles").children().eq(i).hasClass("translated") == false &&   $("#obstacles").children().eq(i).hasClass("bird")  ){
                        get_obs_pos = $("#obstacles").children().eq(i).css("left").split("px").slice(0, -1) 
                        $("#obstacles").children().eq(i).css({"left":""+(get_obs_pos - 1000)+"px","transition":""+( speed_time - 400 )+"ms","transition-timing-function":"linear"}).addClass("translated")
                    }
            }
            else{
                /*if($("#obstacles").children().eq(i).hasClass('bird')){
                ...
                $("#obstacles").children().eq(i).css({"visibility":"hidden"}).addClass("bird-died")
                if($("#obstacles").children().eq(i).hasClass("boss")){
                    $("#obstacles").children().eq(i).removeClass().css({"position":"absolute"})
                }
                /*$("#obstacles").children().eq(i).remove()
...
let remove_zeyed = function(){
    for(i=0;i<obstacle.length;i++){
        if($("#obstacles").children().eq(i).length){
            obstacle_pos_zeyed=$("#obstacles").children().eq(i).position().left
        }
...
let bullets_move = function(){
    for(i=0;i<bullets.length;i++){
        if($("#bullets").children().eq(i).length){
            get_bullet_pos=$("#bullets").children().eq(i).position().left
            if(get_bullet_pos<900){
                if($("#bullets").children().eq(i).hasClass("translated") == false && $("#bullets").children().eq(i).length ){
                    $("#bullets").children().eq(i).css({"left":""+(get_bullet_pos +1000)+"px","transition":"3000ms","transition-timing-function":"linear"}).addClass("translated")
                }
            }
            else{
    ...
    for(j=0;j<bullets.length;j++){
            for(i=0;i<birds.length;i++){
                if($("#bullets").children().eq(j).length && $(".bird").eq(i).length){
                    if(($("#bullets").children().eq(j).position().left  + 40) >= $(".bird").eq(i).position().left && $("#bullets").children().eq(j).position().top <= $(".bird").eq(i).position().top +20 && $("#bullets").children().eq(j).position().top>=  $(".bird").eq(i).position().top - 45 && $(".bird").eq(i).hasClass("bird-died") == false){
                        $(".bird").eq(i).css({"visibility":"hidden"}).removeClass().css({"position":"absolute"})
...
let score_add = function(){
    ...
    if ( score == 500){
        clearInterval(obstacle_move_loop)
        obstacle_move_loop = setInterval(()=>{ obstacle_move() },10.75)
        clearInterval(bird_move_loop)
        bird_move_loop = setInterval(()=>{bird_move();bird_dead()},8.75)
        ...
        setTimeout(()=>{
            if( stop_settimeout == false){
                clearInterval(obstacle_move_loop)
                clearInterval(bird_move_loop)
                obstacle_move_loop = setInterval(()=>{ obstacle_move() },10.5)
                bird_move_loop = setInterval(()=>{bird_move();bird_dead()},8.5)
        ...
        setTimeout(()=>{
            if( stop_settimeout == false){
                clearInterval(obstacle_move_loop)
                clearInterval(bird_move_loop)
                obstacle_move_loop = setInterval(()=>{ obstacle_move() },10.25)
                bird_move_loop = setInterval(()=>{bird_move();bird_dead()},8.25)
        ...
        $("#speeding-msg").show()
        clearInterval(obstacle_move_loop)
        obstacle_move_loop = setInterval(()=>{ obstacle_move() },9.75)
        clearInterval(bird_move_loop)
        bird_move_loop = setInterval(()=>{bird_move();bird_dead()},7.75)
        ...
        setTimeout(()=>{
            if( stop_settimeout == false){
                clearInterval(obstacle_move_loop)
                clearInterval(bird_move_loop)
                obstacle_move_loop = setInterval(()=>{ obstacle_move() },9.5)
                bird_move_loop = setInterval(()=>{bird_move();bird_dead()},7.5)
        ...
        setTimeout(()=>{
            if( stop_settimeout == false){
                clearInterval(obstacle_move_loop)
                clearInterval(bird_move_loop)
                obstacle_move_loop = setInterval(()=>{ obstacle_move() },9.25)
                bird_move_loop = setInterval(()=>{bird_move();bird_dead()},7.25)
        ...
        $("#speeding-msg").show()
        clearInterval(obstacle_move_loop)
        obstacle_move_loop = setInterval(()=>{ obstacle_move() },8.75)
        clearInterval(bird_move_loop)
        bird_move_loop = setInterval(()=>{bird_move();bird_dead()},6.75)
        ...
        setTimeout(()=>{
            if( stop_settimeout == false){
                clearInterval(obstacle_move_loop)
                clearInterval(bird_move_loop)
                obstacle_move_loop = setInterval(()=>{ obstacle_move() },8.5)
                bird_move_loop = setInterval(()=>{bird_move();bird_dead()},6.5)
        ...
        setTimeout(()=>{
            if( stop_settimeout == false){
                clearInterval(obstacle_move_loop)
                clearInterval(bird_move_loop)
                obstacle_move_loop = setInterval(()=>{ obstacle_move() },8.25)
                bird_move_loop = setInterval(()=>{bird_move();bird_dead()},6.25)
        ...
        $("#speeding-msg").show()
        clearInterval(obstacle_move_loop)
        obstacle_move_loop = setInterval(()=>{ obstacle_move() },7.75)
        clearInterval(bird_move_loop)
        bird_move_loop = setInterval(()=>{bird_move();bird_dead()},5.75)
        ...
        setTimeout(()=>{
            if( stop_settimeout == false){
                clearInterval(obstacle_move_loop)
                clearInterval(bird_move_loop)
                obstacle_move_loop = setInterval(()=>{ obstacle_move() },7.5)
                bird_move_loop = setInterval(()=>{bird_move();bird_dead()},5.5)
        ...
        setTimeout(()=>{
            if( stop_settimeout == false){
                clearInterval(obstacle_move_loop)
                clearInterval(bird_move_loop)
                obstacle_move_loop = setInterval(()=>{ obstacle_move() },7.25)
                bird_move_loop = setInterval(()=>{bird_move();bird_dead()},5.25)
        ...
        $("#speeding-msg").show()
        clearInterval(obstacle_move_loop)
        obstacle_move_loop = setInterval(()=>{ obstacle_move() },6.75)
        clearInterval(bird_move_loop)
        bird_move_loop = setInterval(()=>{bird_move();bird_dead()},4.75)
        ...
        setTimeout(()=>{
            if( stop_settimeout == false){
                clearInterval(obstacle_move_loop)
                clearInterval(bird_move_loop)
                obstacle_move_loop = setInterval(()=>{ obstacle_move() },6.5)
                bird_move_loop = setInterval(()=>{bird_move();bird_dead()},4.5)
        ...
        setTimeout(()=>{
            if( stop_settimeout == false){
                clearInterval(obstacle_move_loop)
                clearInterval(bird_move_loop)
                obstacle_move_loop = setInterval(()=>{ obstacle_move() },6.25)
                bird_move_loop = setInterval(()=>{bird_move();bird_dead()},4.25)
...
let start = function(){
    ...
    // Opera + Chrome + Edge
        ...
        setTimeout(()=>{
            obstacle_add()
            obstacle_add_loop = setInterval(()=>{ obstacle_add(); bird_dead() },obstacle_time)
            obstacle_move_loop = setInterval(()=>{ obstacle_move() ; bird_dead()},11)
        },500)
    $("#score").html("00000")
    $("#obstacles").children().remove()
    $("#character").css({"background":"black","background-size":"cover"})
...
let over = function(){
        if( new_chance == false){
            ...
            if( heart_availability == true && boss == false){
...
let get_obs_pos;
let get_obstacle_position = function(){
    for(i=0;i<obstacle.length;i++){
        if($("#obstacles").children().eq(i).hasClass("little1") || $("#obstacles").children().eq(i).hasClass("diam") || $("#obstacles").children().eq(i).hasClass("big") | $("#obstacles").children().eq(i).hasClass("bird") ){
            get_obs_pos = $("#obstacles").children().eq(i).css("left").split("px").slice(0, -1)
            $("#obstacles").children().eq(i).css({"left":""+get_obs_pos+"px","transition":"0ms","transition-timing-function":"linear"}).removeClass("translated")
        }
    }

    for(j=0;j<bullets.length;j++){
        if($("#bullets").children().eq(j).length){
            get_bullet_pos = $("#bullets").children().eq(j).css("left").split("px").slice(0, -1)
            $("#bullets").children().eq(j).css({"left":""+get_bullet_pos+"px","transition":"0s","transition-timing-function":"linear"}).removeClass("translated")   
        }
    }
}
...
let speed_up_bug = function(){
    for(i=0;i<obstacle.length;i++){
        $("#obstacles").children().eq(i).removeClass("translated")
    }
}
