...
let run = function(){
    if(character_etat=="running"){
        clearInterval(jump)
        if (run_cycle != 6){
            $("#character").css({"background":"url('media/main_run.gif')","background-size":"cover"})
            run_cycle++
        }
        else{
            run_cycle = 0
...
let jump = function(){
    if(character_etat=="jumping"){
        ...
        if(jump_etat==1){
            $("#character").css({"background":"url('media/main_jump_up.png')","background-size":"cover"})
            ...
            $("#character").css({"background":"url('media/main_jump_down.png')","background-size":"cover"})
...
let obstacle_add = function(){
    ...
    if(type_obstacle>=0 && type_obstacle<10){
...
let score_add = function(){
    ...
    if( score == 100 ){
        clearInterval(obstacle_move_loop)
        clearInterval(bird_move_loop)
        obstacle_move_loop = setInterval(()=>{ obstacle_move() },11.75)
        bird_move_loop = setInterval(()=>{bird_move();bird_dead()},9.75)
        obstacle_move_key = 11.75
        bird_move_key = 9.75
        setTimeout(()=>{
            if( stop_settimeout == false){
                clearInterval(obstacle_move_loop)
                clearInterval(bird_move_loop)
                obstacle_move_loop = setInterval(()=>{ obstacle_move() },11.5)
                bird_move_loop = setInterval(()=>{bird_move();bird_dead()},9.5)
                obstacle_move_key = 11.5
                bird_move_key = 9.5
            }
        },1000)
        setTimeout(()=>{
            if( stop_settimeout == false){
                clearInterval(obstacle_move_loop)
                clearInterval(bird_move_loop)
                obstacle_move_loop = setInterval(()=>{ obstacle_move() },11.25)
                bird_move_loop = setInterval(()=>{bird_move();bird_dead()},9.25)
                obstacle_move_key = 11.25
                bird_move_key = 9.25
            }
        },2000)
        setTimeout(()=>{
...
let start = function(){
    ...
    $("#score").html("00000")
    $("#obstacles").children().remove()
    $("#character").css({"background":"url('media/run/main (4).png')","background-size":"cover"})
...
let over = function(){
        if( new_chance == false){
            ...
            $("#character").css({"background":"url('media/main_idle.png')","background-size":"cover"})
            $("#boss-bar").hide()
            clearInterval(obstacle_add_loop)
            clearInterval(obstacle_move_loop)
            clearInterval(obstacle_add_loop2)
